...
AccountType = Literal[
    # US account types
    "401k", "roth_ira", "traditional_ira", "taxable", "529", "hsa", "pension",
    # India account types
    "indian_epf", "indian_vpf", "indian_ppf", "indian_nps", "indian_apy",
...
HealthcarePreference = Literal["private", "mixed", "government"]

TaxRegimePreference = Literal["old", "new", "compare"]
...
class UserCreate(BaseModel):
    ...
    target_retirement_income: Optional[Decimal] = Field(
        None, description="Annual income goal in retirement (in user's currency)", ge=0, decimal_places=2
    )
    ...
    country_code: Optional[CountryCode] = Field(default="IN", description="Country code (IN or US)")
    currency_code: Optional[CurrencyCode] = Field(default="INR", description="Currency code (INR or USD)")
    tax_regime_preference: Optional[TaxRegimePreference] = Field(
        default="new", description="India tax regime preference: old, new, or compare"
    )
    annual_salary_income: Optional[Decimal] = Field(default=Decimal("0"), description="Annual salary income", ge=0)
    annual_business_income: Optional[Decimal] = Field(default=Decimal("0"), description="Annual business income", ge=0)
    annual_other_income: Optional[Decimal] = Field(default=Decimal("0"), description="Annual other income", ge=0)
    deductions_80c: Optional[Decimal] = Field(default=Decimal("0"), description="Section 80C deductions", ge=0)
    deductions_80d: Optional[Decimal] = Field(default=Decimal("0"), description="Section 80D deductions", ge=0)
    nps_80ccd1b: Optional[Decimal] = Field(default=Decimal("0"), description="NPS 80CCD(1B) deduction", ge=0)
    hra_claim: Optional[Decimal] = Field(default=Decimal("0"), description="HRA claim amount", ge=0)
    home_loan_interest: Optional[Decimal] = Field(default=Decimal("0"), description="Home loan interest deduction", ge=0)
    city_tier: Optional[CityTier] = Field(default="tier_1", description="City tier: tier_1, tier_2, tier_3")
    healthcare_preference: Optional[HealthcarePreference] = Field(
        default="mixed", description="Healthcare preference: private, mixed, government"
    )
    expected_family_support_ratio: Optional[float] = Field(
        default=0.0, description="Expected family support ratio (0.0-1.0)", ge=0, le=1
    )
    dependent_parents_count: Optional[int] = Field(
        default=0, description="Number of dependent parents", ge=0, le=4
    )
    expected_post_retirement_family_obligations: Optional[Decimal] = Field(
        default=Decimal("0"), description="Expected post-retirement family obligations (annual)", ge=0
    )
    fixed_income_preference: Optional[int] = Field(
        default=50, description="Fixed income preference (0-100)", ge=0, le=100
    )
    gold_preference: Optional[int] = Field(
        default=20, description="Gold preference (0-100)", ge=0, le=100
    )
    guaranteed_income_priority: Optional[int] = Field(
        default=50, description="Guaranteed income priority (0-100)", ge=0, le=100
    )
