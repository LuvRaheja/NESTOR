...
def setup_test_data(db):
    ...
    if not accounts:
        account_data = AccountCreate(
            clerk_user_id=test_user_id,
            account_name="EPF",
            account_type="indian_epf",
            cash_balance=50000.00
        )
        account_id = db.accounts.create(account_data.model_dump())
        print(f"  ✓ Created test account: EPF")
        
        # Add some positions
        positions = [
            {'symbol': 'NIFTYBEES', 'quantity': 500},
            {'symbol': 'JUNIORBEES', 'quantity': 100},
            {'symbol': 'LIQUIDBEES', 'quantity': 50},
            {'symbol': 'GOLDBEES', 'quantity': 200}
        ]
